...
Global["config"]["date_format"] = "%m-%d-%Y %I:%M%p"
...
def preBuild(site):
	...
	for page in site.pages():
		...
		if page.path.startswith("%s/" % Global["config"]["path"]):
			...
			logging.info("Processing page %s" % (page.path))

			context = page.context()
			context_post = {"path": page.path}

			if context.has_key('draft'):
				continue
			...
			for field in ["title", "date", "tags"]:
				...
				if not context.has_key(field):
					...
					if field == "date":
						...
						context_post[field] = context[field].split(",")
						context_post[field].sort()
					else:
